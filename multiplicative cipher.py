...
key = int(input("Enter the value to be used for Cipher: "))
if(coprime(key,26)==False):
...
for word in iter_text:
  for letter in word:
    ...
    #adding 64 back to the new_letter to get the ascii equivalent character 
      new_letter = chr(int((((ord(letter)-65)*key)%26)+65))
      encrypted_word += new_letter
  encrypted_word += " "
print("The Cipher Text through Multiplicative Cipher is:",encrypted_word)
 
#Decryption
 
iter_ciphertext = encrypted_word.split(" ")
decrypted_word = " " 
inverse_key = modInverse(key,26)
for word in iter_ciphertext:
  for letter in word:
    old_letter = chr(int((((ord(letter)-65)*inverse_key)%26)+65))
